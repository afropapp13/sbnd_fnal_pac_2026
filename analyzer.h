...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   analyzer(TString in, TString out, TString weight = "", int index = -1, TTree *tree=0);
   virtual ~analyzer();
...
#ifdef analyzer_cxx
analyzer::analyzer(TString InputFile, TString OutputFile, TString Weights, int index, TTree *tree) : fChain(0) 
{
   ...
   if (tree == 0) {
      ...
      if (!f || !f->IsOpen()) {
         f = TFile::Open(FullName,"readonly");
      }
